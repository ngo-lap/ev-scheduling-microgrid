...
timeMatrix(sub2ind(size(socMatrix), tArrival, 1:nSockets)) = 1;
timeMatrix(sub2ind(size(socMatrix), tDeparture, 1:nSockets)) = -1;
...
figure
heatmap( ...
    round(value(ev)), 'XLabel', 'Vehicles', 'YLabel', 'Time (t)', 'Title', 'ev', ...
    'ColorbarVisible', 'off' ...
    )
